...
You're given a directed weighted graph, represented by an array of edges. Each edge signifies a one-way connection between two locations, complete with a cost. Your mission is to create a "Graph" class that can efficiently manage this network and provide two essential services.
    ...
    1) Setting Up the Map (Graph(int n, int[][] edges)):
        *Just like preparing a map for a journey, you initialize the graph with "n" locations and the given edges. This step lays out the foundation for your navigation.
    2) Plotting New Routes (addEdge(int[] edge)):
        * As your journey unfolds, you'll come across new routes. The "addEdge" method allows you to add these paths to your map. It's worth noting that this method ensures no duplicate paths between locations.
    3) Finding the Optimal Path (int shortestPath(int node1, int node2)):
        * The core of this adventure lies in discovering the shortest and most cost-effective path from one location to another. This method calculates the minimum cost required to travel from "node1" to "node2." If a path doesn't exist, it signals your GPS to return -1.
...
1)Dijkstra's Algorithm
    Intuition
        If you are not familiar with Dijkstra's Algorithm, please refer to our explore cards Dijkstra's Algorithm. We will focus on the usage in this article and not the underlying principles or implementation details.
    ...
    * Time complexity:
        * O(N+M⋅(V+E⋅logV))
        * initialization:
            * O(E+V). Initializing a list to the size of V costs O(V) and iterating over all the edges costs O(E)
        * addEdge:
            * O(N). Appending an element to a list costs O(1), and when this operation is performed N times, it results in a linear time complexity of O(N).
        * shortestPath:
            * O(M⋅(V+E⋅logV)).
            * Initializing the costForNode list will incur a time complexity of O(V). The time complexity for Dijkstra's algorithm is (E⋅logV). Calling shortestPath M times leads to a combined time complexity of O(M⋅(V+E⋅logV)).

    * Space complexity:
        * O(E+V+N)
        * initialization:
            * O(E+V). This is the cost to initialize the adjacency list.
        ...
        * shortestPath:
            * O(E+V). The costForNode list will incur a space complexity of O(V). The priority queue will will incur a space complexity of O(E).
    ...
    2)Floyd–Warshall algorithm
        Intuition
            ...
            Floyd-Warshall Algorithm:
                * We recognize the need to find the shortest paths between all pairs of nodes efficiently. To achieve this, we implement the Floyd-Warshall algorithm. We use nested loops to iterate through all possible intermediate nodes (k), source nodes (i), and destination nodes (j). For each pair of nodes (i, j), we update the minimum cost if there's a shorter path through the intermediate node (k).
        ...
        * Time complexity:
            * O(M+N⋅V^2+V^3)
        * initialization:
            * O(V^3). The Floyd-Warshall Algorithm incurs a cost of O(V^3) to find the minimum cost between all pairs of vertices.
        * addEdge:
            * O(N⋅V^2). When adding an edge, we iterate over the whole matrix to check if the new edge lowers the cost between any of the vertices. This operation costs O(V^2). When this operation is performed N times, it results in a time complexity of O(N⋅V^2).
        * shortestPath:
            * O(M). Finding the shortestPath doesn't require any additional computation. Hence, it incurs a constant time complexity of O(1). When this operation is performed M times, it results in a linear time complexity of O(M).
        *Notes:
...
private:
    ...
    priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> list_nodes;
...
public:
    Graph(int n, vector<vector<int>> &edges)
    {
        ...
        for (size_t i = 0; i < edges.size(); i++)
            my_graph[edges[i][0]].push_back({edges[i][2], edges[i][1]});
    }

    void addEdge(vector<int> edge)
    {
        my_graph[edge[0]].push_back({edge[2], edge[1]});
    }
    ...
    int shortestPath(int node1, int node2)
    {
        ...
        dimension[node1] = 0;
        list_nodes.push({0, node1});
        while (!list_nodes.empty())
        {
            const pair<int, int> top_node = list_nodes.top();
            list_nodes.pop();
            if (top_node.second == node2)
                return (top_node.first);
            if (top_node.first > dimension[top_node.second])
                continue;
            for (const pair<int, int> &current_node : my_graph[top_node.second])
            {
                sum = current_node.first + top_node.first;
                if (sum < dimension[current_node.second])
                {
                    dimension[current_node.second] = sum;
                    list_nodes.push({sum, current_node.second});
                }
            }
        }
        return (-1);
...
{
    {
        ...
        edges = {{0, 1, 10}, {0, 2, 3}, {2, 1, 2}, {1, 3, 5}, {2, 3, 20}, {3, 4, 1}};

        g = new Graph(5, edges);
        g->addEdge({0, 4, 2});
        cout << "result = " << g->shortestPath(0, 4) << endl;
        g->addEdge({4, 0, 7});
        cout << "result = " << g->shortestPath(4, 2) << endl;
    ...
    }
    {
        ...
        g = new Graph(4, edges);
        cout << "result = " << g->shortestPath(0, 3) << endl;
