...
Approach 1:
    * Convert to Graph with Breadth First Search
    Intuition
...
Approach 2:
    * Convert to Graph with Depth First Search
    Intuition
    ...
    * Upon completing the DFS traversal, the binary tree is fully constructed and linked according to the relationships defined in the descriptions array. Finally, we return the root node of the constructed binary tree.
    ...
    Helper method dfs with parameters:
...
Approach 3:
    * Constructing Tree From Directly Map and TreeNode Object
    Intuition
...
private:
    ...
    void building(TreeNode *&head)
    {
        if (!graph.count(head->val))
            return;
        if (graph[head->val].first != 0)
        {
            head->left = new TreeNode(graph[head->val].first);
            building(head->left);
        }
        if (graph[head->val].second != 0)
        {
            head->right = new TreeNode(graph[head->val].second);
            building(head->right);
        }
    }

    int set_up_all(const vector<vector<int>> &descriptions)
    {
        graph.clear();
        child.clear();
        for (const vector<int> &triplet : descriptions)
        {
            if (triplet[2] == 1)
                graph[triplet[0]].first= triplet[1];
            else
                graph[triplet[0]].second = triplet[1];
            child.insert(triplet[1]);
        }
        for (const pair<const int, pair<int, int>> &root : graph)
        {
...
public:
    ...
    {
        TreeNode *head = new TreeNode(set_up_all(descriptions));
        if (head->val == -1)
            return (nullptr);
        building(head);
        return (head);
...
void print_tree(TreeNode *Tree, int depth = 0)
{
    if (!Tree)
        return;
...
void print_and_delete(TreeNode *&Tree)
{
    if (!Tree)
        return;
...
int main()
{
    ...
    descriptions = {{20, 15, 1}, {20, 17, 0}, {15, 10, 1}, {15, 12, 0}, {17, 18, 1}, {17, 19, 0}};
    Tree = s.createBinaryTree(descriptions);
    print_and_delete(Tree);

    descriptions = {{5, 3, 1}, {3, 2, 1}, {2, 1, 1}, {5, 8, 0}};
    Tree = s.createBinaryTree(descriptions);
    print_and_delete(Tree);

    descriptions = {{15, 10, 1}, {20, 17, 0}, {17, 19, 0}, {20, 15, 1}, {17, 18, 1}};
    Tree = s.createBinaryTree(descriptions);
    print_and_delete(Tree);
}